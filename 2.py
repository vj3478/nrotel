...
async def fetch_dashboard_guids(session: aiohttp.ClientSession) -> List[str]:
    """Get list of dashboard GUIDs using entitySearch with pagination."""
    ...
    for account_id in ACCOUNT_IDS:
        ...
        while True:
            ...
            payload = {
                ...
                query {{
                  actor {{
                    entitySearch(query: "type='DASHBOARD' AND accountId = {account_id}"{cursor_arg}) {{
                      results {{
                        entities {{
                          guid
                          name
            ...
            try:
                ...
                if not data or "data" not in data or "actor" not in data["data"]:
                    logger.error(f"Invalid response format from GraphQL API for account {account_id}")
                    break

                results = data["data"]["actor"]["entitySearch"]["results"]
                for entity in results["entities"]:
                    guids.append(entity["guid"])
                    logger.debug(f"Found dashboard: {entity['name']} ({entity['guid']})")
                cursor = results.get("nextCursor")
                if not cursor:
                    break
            except Exception as e:
                logger.error(f"Error fetching dashboard GUIDs for account {account_id}: {str(e)}")
                break
    return guids
...
async def run(mode: str, single_guid: Optional[str] = None, session: Optional[aiohttp.ClientSession] = None) -> None:
    ...
    try:
        ...
        if not all(isinstance(dash, dict) for dash in results):
            raise ValueError("All results must be dictionaries")
        if not all("dashboard_guid" in dash and "result" in dash for dash in results):
            raise ValueError("All results must contain 'dashboard_guid' and 'result' keys")
        ...
        try:
            ...
            csv_path = os.path.join(OUTPUT_DIR, "validated_results.csv")
            logger.info(f"Writing CSV file with {len(results)} dashboards")
            ...
            try:
                ...
                required_fields = ["dashboard_guid", "dashboard_name", "widget_id"]
                ...
                if BATCH_SIZE > 10000:
                    logger.warning(f"Batch size {BATCH_SIZE} is very large, consider reducing it")
                
                total_batches = (len(results) + BATCH_SIZE - 1) // BATCH_SIZE
                logger.info(f"Processing {len(results)} dashboards in {total_batches} batches of {BATCH_SIZE}")
                ...
                for i in range(0, len(results), BATCH_SIZE):
                    ...
                    for dash in batch:
                        ...
                        if not has_permissions:
                            ...
                            try:
                                layout_json = json.dumps(dashboard_data.get("layout", {}))
                            except Exception as e:
                                logger.error(f"Error encoding layout JSON for dashboard {guid}: {e}")
                                layout_json = "{}"
                            
                            try:
                                variables_json = json.dumps(dashboard_data.get("variables", []))
                            except Exception as e:
                                logger.error(f"Error encoding variables JSON for dashboard {guid}: {e}")
                                variables_json = "[]"
                            ...
                            missing_fields = [field for field in fieldnames if field not in row_data]
                            if missing_fields:
                                logger.warning(f"Missing fields in no-permissions row data: {missing_fields}")
                        ...
                        for page in dashboard_data.get("pages", []):
                            for widget in page.get("widgets", []):
                                ...
                                if not owner:
                                    logger.warning(f"No owner data found for dashboard {guid}")
                                
                                try:
                                    layout_json = json.dumps(widget.get("layout", {}))
                                except Exception as e:
                                    logger.error(f"Error encoding layout JSON for widget {widget.get('id')}: {e}")
                                    layout_json = "{}"
                                ...
                                missing_fields = [field for field in fieldnames if field not in row_data]
                                if missing_fields:
                                    logger.warning(f"Missing fields in row data: {missing_fields}")
                                    for field in missing_fields:
                                        row_data[field] = ""
