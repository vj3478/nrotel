    ...
    for account_name, dashboards in dashboards_by_account.items():
        ...
        with open(filename, mode='w', newline='') as f:
            fieldnames = [
                ...
                "page_name", "page_guid", "widget_title", "widget_id", "visualization_id",
                "layout", "variables", "old_query", "new_query", "validation_result"
            ]
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            ...
            for guid, data in dashboards:
                ...
                for page in data.get("pages", []):
                    ...
                    for widget in page.get("widgets", []):
                        ...
                        viz_id = cfg.get("visualization", {}).get("id") or widget.get("visualization", {}).get("id")
