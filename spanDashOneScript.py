...
async def graphql_query(session, payload):
    ...
    async with session.post("https://api.newrelic.com/graphql", headers=headers, json=payload) as resp:
        if resp.status != 200:
            log(f"GraphQL error: {resp.status}")
            return None
...
async def run(mode, single_guid=None):
    ...
    for guid in guids:
        ...
        account_id = dashboard.get("permissions", [{}])[0].get("accountId", 0)
        ...
        for page in dashboard.get("pages", []):
            for widget in page.get("widgets", []):
                ...
                if any(k.lower() in query_lower for k in BLOCK_KEYWORDS):
                ...
                if any(k.lower() in query_lower for k in SKIP_KEYWORDS):
                ...
                batch.append({"query": transformed, "accountId": account_id, "widget": widget, "page": page, "original": query})
                count_total += 1
        ...
        if mode == "update" and count_valid == count_total and count_fail == 0:
            ...
            if updated_widgets:
                for attempt in range(3):
                    ...
                    log(f"❌ Exception while updating widgets for {dashboard['guid']}: {e}")
...
if __name__ == "__main__":
    ...
    if args.mode == "update" and not args.confirm:
        print("This will modify live New Relic dashboards.")
